...
async def _save_sources(ctx: _IngestContext, query: str, limit: int) -> list[Source]:
    ...
    sources = [source for source in extracted if is_probably_russian(source.name, source.metadata.get("description"))]
    logger.info("Language filter kept %d/%d source(s)", len(sources), len(extracted))
    saved_sources = []
...
async def _extract_documents_for_source(ctx: _IngestContext, source: Source, limit: int) -> int:
    ...
    # Drop foreign-language comment threads a Russian video still attracts, same gate as sources.
